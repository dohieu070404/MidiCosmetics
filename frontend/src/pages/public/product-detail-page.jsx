...
function DetailBlock({ title, value, html = false }) {
  ...
  return (
    <details className="group border-t border-border py-5" open>
      <summary className="cursor-pointer list-none font-display text-xl font-normal">
        {title}
      </summary>
      {html ? (
        <div
          className="prose mt-4 max-w-none break-words text-[0.96rem] leading-7 text-muted-foreground"
          dangerouslySetInnerHTML={{ __html: value }}
        />
      ) : (
        <p className="mt-4 whitespace-pre-wrap text-[0.96rem] leading-7 text-muted-foreground">
          {value}
        </p>
      )}
    </details>
...
export function ProductDetailPage() {
  ...
  useEffect(() => {
    ...
    publicApi
      ...
      .then((response) => {
        ...
        if (item)
          publicApi
            .trackInterest({ eventType: 'PRODUCT_VIEWED', productUuid: item.uuid || item.id })
            .catch(() => null);
      })
      .catch((err) => {
  ...
  if (error && !product)
    return (
      <Container className="py-16">
        <StatePanel type="error" title="Không mở được sản phẩm" description={error} />
      </Container>
    );
  if (!product)
    return (
  ...
  return (
    <div className="pb-24 lg:pb-36">
      <Container className="py-6 text-xs uppercase tracking-[0.1em] text-muted-foreground">
        <Link to={ROUTE_PATHS.products} className="transition-colors hover:text-foreground">
          Sản phẩm
        </Link>{' '}
      ...
      <Container className="grid gap-10 lg:grid-cols-[1.08fr_.92fr] lg:gap-16">
        ...
        <div className="lg:sticky lg:top-32 lg:self-start">
          <p className="midi-eyebrow">
            {product.category?.name || product.brand?.name || 'Midi Cosmetics'}
          </p>
          <h1 className="mt-5 font-display text-5xl font-normal leading-[.94] tracking-[-0.055em] sm:text-6xl">
            {product.name}
          </h1>
          ...
          <div className="mt-7 grid grid-cols-[auto_1fr] gap-3">
            <QuantityStepper
              ...
              max={product.stock ? Math.min(20, product.stock) : 20}
            />
            <Button
              type="button"
              onClick={add}
              disabled={unavailable}
              className="midi-link-arrow w-full"
            >
              {unavailable ? 'Tạm hết hàng' : 'Thêm vào giỏ'}{' '}
              {!unavailable ? <ShoppingBag /> : null}
            </Button>
          </div>
          <p className="mt-4 flex items-center gap-2 text-sm text-muted-foreground">
            <Check className="size-4 text-primary" /> Không cần tài khoản · Tạo phiếu rồi gửi qua
            Messenger
          </p>
          <div className="mt-8">
          ...
          <Button asChild variant="outline" className="midi-link-arrow mt-6">
            <Link to={ROUTE_PATHS.cart}>
              Xem giỏ và tạo phiếu <ArrowRight />
            </Link>
          </Button>
        </div>
      </Container>
      {data.related?.length ? (
        <Container className="mt-24">
          <div className="flex items-end justify-between border-b border-border pb-7">
            <div>
              <p className="midi-eyebrow">Có thể bạn sẽ thích</p>
              <h2 className="mt-3 font-display text-4xl font-normal tracking-[-0.045em]">
                Sản phẩm liên quan
              </h2>
            </div>
          </div>
          <div className="mt-8 grid gap-x-4 gap-y-10 sm:grid-cols-2 lg:grid-cols-4">
            {data.related.slice(0, 4).map((item) => (
              <ProductCard key={item.uuid || item.id} product={item} />
            ))}
          </div>
        </Container>
      ) : null}
