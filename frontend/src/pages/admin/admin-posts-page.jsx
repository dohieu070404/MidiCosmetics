...
export function AdminPostsPage() {
  ...
  const toggleTag = (uuid) =>
    setForm((old) => ({
      ...old,
      tagUuids: old.tagUuids.includes(uuid)
        ? old.tagUuids.filter((item) => item !== uuid)
        : [...old.tagUuids, uuid],
    }));
  const thumbnailPreview = useMemo(
    () => (thumbnail ? URL.createObjectURL(thumbnail) : currentThumbnailUrl),
    [thumbnail, currentThumbnailUrl],
  );
  useEffect(
    () => () => {
      if (thumbnail && thumbnailPreview) URL.revokeObjectURL(thumbnailPreview);
    },
  ...
  const uploadThumbnail = async () => {
    ...
    fd.append('altText', form.title);
    const res = await adminApi.uploadImage(fd);
    return res.data.media.uuid;
  };
  ...
  const cancel = () => {
    if (isDirty && !window.confirm('Bạn có thay đổi chưa lưu. Rời form và bỏ các thay đổi?'))
      return;
    setEditing(null);
    setForm(initial);
    setFormBaseline(JSON.stringify(initial));
  ...
  async function submit(e) {
    ...
    try {
      ...
      if (editing) {
        await adminApi.updateBlogPost(editing, payload);
        setOk('Đã cập nhật bài viết.');
      } else {
        await adminApi.createBlogPost(payload);
        setOk('Đã tạo bài viết.');
      }
      await load();
      setEditing(null);
      setForm(initial);
  ...
  const toggleFeatured = async (r) => {
    ...
    try {
      ...
      await adminApi.updateBlogPostFeatured(r.uuid, {
        isFeatured: next,
  ...
  return (
    <div className="grid gap-6">
      <PageHeader
        ...
        description="Viết bài, xuất bản và bật/tắt bài viết đề xuất trang chủ ngay trong bảng."
        actions={
          <>
      ...
      <TabButtons
        value={view}
        onChange={(next) => (next === 'form' && !editing ? openCreate() : setView(next))}
        items={[
          { value: 'list', label: 'Bài viết' },
          { value: 'form', label: editing ? 'Sửa bài' : 'Viết bài' },
        ]}
      />
          ...
          <form onSubmit={submit} className="grid gap-5">
            ...
            <SectionCard title="Thông tin bài viết">
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <FileInput
                  ...
                  onChange={(e) => {
                    ...
                    const fileError = validateLocalImageFile(file, 'Ảnh đại diện');
                    if (fileError) {
                      setError(fileError);
                      e.target.value = '';
                      setThumbnail(null);
                  ...
                  hint="Chỉ hỗ trợ JPG, PNG, WEBP, tối đa 5MB. Chọn ảnh mới nếu muốn thay ảnh hiện tại."
                />
                <div className="flex items-end">
                  <MediaPicker
                    label="Chọn cover từ thư viện"
                    onSelect={(media) => {
                      setThumbnail(null);
                      setCurrentThumbnailUrl(media.secureUrl);
                      setForm((current) => ({ ...current, featuredImageUuid: media.uuid }));
                    }}
                  />
                </div>
              </div>
              {thumbnailPreview ? (
                <div className="mt-4 rounded-2xl border border-border p-3">
                  <p className="mb-2 text-sm font-medium">Ảnh cover đang chọn</p>
                  <ImageWithFallback
            ...
            <SectionCard title="Nội dung">
              <div className="grid gap-4">
                ...
                <RichTextEditor
                  ...
                  required
                  hint="Dùng toolbar để định dạng. Khi dán nội dung từ bên ngoài, hệ thống chỉ lấy text để tránh HTML độc hại; backend vẫn sanitize trước khi lưu."
                />
              </div>
            </SectionCard>
                  ...
                  {tags.length ? (
                    tags.map((tag) => (
                      ...
                      >
                        <input
                          type="checkbox"
                          checked={form.tagUuids.includes(tag.uuid)}
            ...
            {editing && form.status === 'PUBLISHED' ? (
              <div className="rounded-2xl border border-border bg-secondary/30 p-4 text-sm">
                Bài đã đăng có thể xem công khai tại{' '}
                <a
                  className="font-semibold text-primary underline"
                  href={`/blog/${posts.find((item) => item.uuid === editing)?.slug || ''}`}
                  target="_blank"
                  rel="noreferrer"
                >
                  trang public
      ...
      {view === 'list' ? (
        <SectionCard
          ...
          description="Bài nháp hoặc đã ẩn sẽ không hiển thị public dù được đánh dấu đề xuất."
        >
          <div className="grid gap-4">
            <Toolbar>
              <TextInput
              ...
              <div className="flex items-end">
                <ActionButton type="button" onClick={openCreate} className="w-full">
                  + Viết bài
                </ActionButton>
              </div>
            </Toolbar>
            <AdminTable
              empty="Chưa có bài viết phù hợp bộ lọc."
              columns={[
                {
                  key: 'image',
                  label: 'Ảnh',
              ...
              actions={(r) => (
                <div className="flex flex-wrap gap-2">
                  ...
                  {r.status === 'PUBLISHED' ? (
                    <SecondaryButton
                      type="button"
                      onClick={() => adminApi.unpublishBlogPost(r.uuid).then(load)}
                    >
                      Chuyển nháp
                    </SecondaryButton>
                  ) : (
                    <ActionButton
                      type="button"
                      onClick={() => adminApi.publishBlogPost(r.uuid).then(load)}
                    >
                      Đăng
                    </ActionButton>
                  )}
                  <SecondaryButton
                    type="button"
                    onClick={() => adminApi.archiveBlogPost(r.uuid).then(load)}
                  >
                    Ẩn
                  </SecondaryButton>
                  <DangerButton
                    type="button"
                    onClick={() =>
                      confirm('Xóa bài viết này?') && adminApi.deleteBlogPost(r.uuid).then(load)
                    }
                  >
                    Xóa
                  </DangerButton>
