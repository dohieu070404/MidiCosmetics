  ...
  const verifyChange = async (event) => {
    ...
    try {
      ...
      navigate(ROUTE_PATHS.adminLogin, {
        replace: true,
        state: { notice: 'Mật khẩu đã đổi thành công. Vui lòng đăng nhập lại.' },
      });
    } catch (err) {
  ...
  return (
    <div className="grid gap-6">
      <PageHeader
        ...
        description="Xem hồ sơ admin và đổi mật khẩu bằng mã xác minh qua email."
      />
      ...
      <SectionCard
        ...
        description="Thông tin này chỉ dành cho admin đang đăng nhập."
      >
        {loading ? (
          ...
          <dl className="grid gap-4 text-sm sm:grid-cols-2 lg:grid-cols-3">
            <div className="rounded-2xl bg-secondary/40 p-4">
              ...
              <dd className="mt-1 font-medium">{displayProfile.email || '-'}</dd>
            </div>
            <div className="rounded-2xl bg-secondary/40 p-4">
              <dt className="text-muted-foreground">Tên hiển thị</dt>
              <dd className="mt-1 font-medium">{displayProfile.fullName || '-'}</dd>
            </div>
            <div className="rounded-2xl bg-secondary/40 p-4">
              <dt className="text-muted-foreground">Role</dt>
              <dd className="mt-1 font-medium">{displayProfile.role || '-'}</dd>
            </div>
            <div className="rounded-2xl bg-secondary/40 p-4">
              <dt className="text-muted-foreground">Trạng thái</dt>
              <dd className="mt-1 font-medium">{displayProfile.status || '-'}</dd>
            </div>
            <div className="rounded-2xl bg-secondary/40 p-4">
              <dt className="text-muted-foreground">Ngày tạo</dt>
              <dd className="mt-1 font-medium">{formatDate(displayProfile.createdAt)}</dd>
            </div>
            <div className="rounded-2xl bg-secondary/40 p-4">
              <dt className="text-muted-foreground">Lần đăng nhập gần nhất</dt>
              <dd className="mt-1 font-medium">{formatDate(displayProfile.lastLoginAt)}</dd>
            </div>
          </dl>
        )}
      </SectionCard>
      ...
      <SectionCard
        title="Đổi mật khẩu"
        description="Mật khẩu chỉ được đổi sau khi nhập mã xác minh gửi tới email admin."
      >
        <div className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
          <form className="grid gap-4" onSubmit={requestChange}>
            ...
            <ActionButton type="submit" disabled={saving}>
              {saving ? 'Đang gửi yêu cầu...' : 'Gửi mã xác minh qua email'}
            </ActionButton>
          </form>
          ...
          <div className="rounded-3xl border border-border bg-secondary/30 p-4">
            ...
            <p className="mt-4 text-xs leading-5 text-muted-foreground">
              Hệ thống không lưu mật khẩu thô. Mã xác minh hết hạn sau 15 phút và chỉ dùng được một
              lần.
            </p>
          </div>
        </div>
      ...
      <SectionCard
        ...
        description="Nhập mã xác minh đã được gửi tới email admin. Sau khi đổi thành công, bạn cần đăng nhập lại."
      >
        <form className="grid gap-4 md:grid-cols-[1fr_auto]" onSubmit={verifyChange}>
          <TextInput
            ...
            onChange={updateVerifyForm}
            required
            disabled={!verifyRequested && !verifyForm.token}
            hint={
              !verifyRequested
                ? 'Gửi yêu cầu đổi mật khẩu trước, rồi nhập mã nhận được qua email.'
                : 'Dán mã xác minh từ email.'
            }
          />
          <div className="flex items-end">
            <ActionButton
              type="submit"
              disabled={verifying || (!verifyRequested && !verifyForm.token)}
            >
              {verifying ? 'Đang xác minh...' : 'Xác minh và đổi mật khẩu'}
            </ActionButton>
          </div>
        </form>
      </SectionCard>
