...
const buildStorage = (resolveDestination) => multer.diskStorage({
  destination(req, file, callback) {
    try { callback(null, resolveDestination()); } catch (error) { callback(error); }
  },
...
const validateXlsxArchiveStructure = (buffer) => {
  ...
  for (let index = 0; index < entryCount; index += 1) {
    if (offset + 46 > buffer.length || buffer.readUInt32LE(offset) !== ZIP_CENTRAL_SIGNATURE) return false;
...
export const validateUploadedXlsxFile = async (req, res, next) => {
  try {
    ...
    if (!archive || !validateXlsxArchiveStructure(archive)) {
      await removeRejectedFile(req.file);
      return next(ApiError.badRequest('File import phải là .xlsx hợp lệ, không vượt giới hạn giải nén và không chấp nhận CSV/XLS đổi đuôi'));
    }
    return next();
  } catch (error) {
    await removeRejectedFile(req.file);
