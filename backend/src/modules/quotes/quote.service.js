...
const quoteCode = () =>
  `MD-${new Date().toISOString().slice(2, 10).replaceAll('-', '')}-${crypto.randomBytes(3).toString('hex').toUpperCase()}`;
const serializeNumber = (value) => Number(value?.toString?.() ?? value ?? 0);
const DAILY_QUOTE_LIMIT = 10;

const sessionHashForRequest = (req) =>
  hashToken(`${req?.ip || 'unknown'}\0${String(req?.headers?.['user-agent'] || '').slice(0, 240)}`);
const recaptchaError = (statusCode, message, code) =>
  new ApiError(statusCode, message, [{ field: 'recaptchaToken', code, message }], { code });
const expectedRecaptchaHostnames = () => {
  const hostnames = new Set(env.recaptcha.allowedHostnames);
  try {
    if (env.frontendUrl)
      hostnames.add(new URL(env.frontendUrl).hostname.toLowerCase().replace(/\.$/, ''));
  } catch {
...
const verifyRecaptcha = async (token, req) => {
  ...
  if (!env.recaptcha.secretKey)
    throw recaptchaError(
      503,
      'Google reCAPTCHA chÆ°a Ä‘Æ°á»£c cáº¥u hĂ¬nh trĂªn mĂ¡y chá»§.',
      'RECAPTCHA_NOT_CONFIGURED',
    );
  if (!token)
    throw recaptchaError(
      422,
      'Vui lĂ²ng hoĂ n thĂ nh Google reCAPTCHA trÆ°á»›c khi táº¡o phiáº¿u.',
  ...
  try {
    ...
    throw recaptchaError(
      503,
      'KhĂ´ng káº¿t ná»‘i Ä‘Æ°á»£c Google reCAPTCHA. Vui lĂ²ng thá»­ láº¡i sau.',
  ...
  if (!result?.success) {
    logger.warn(
      { requestId: req?.id, providerCodes },
      'Google reCAPTCHA rejected quote verification',
    );
    if (
      providerCodes.some((code) => ['missing-input-secret', 'invalid-input-secret'].includes(code))
    ) {
      throw recaptchaError(
        503,
        'Secret key Google reCAPTCHA phĂ­a mĂ¡y chá»§ khĂ´ng há»£p lá»‡ hoáº·c khĂ´ng cĂ¹ng cáº·p vá»›i Site key.',
        'RECAPTCHA_SERVER_CONFIG_INVALID',
      );
    }
    if (providerCodes.includes('timeout-or-duplicate')) {
      throw recaptchaError(
        422,
        'Google reCAPTCHA Ä‘Ă£ háº¿t háº¡n hoáº·c Ä‘Ă£ Ä‘Æ°á»£c sá»­ dá»¥ng. Vui lĂ²ng xĂ¡c nháº­n láº¡i.',
        'RECAPTCHA_TOKEN_EXPIRED',
      );
    }
    throw recaptchaError(
      422,
      'Google reCAPTCHA khĂ´ng há»£p lá»‡. Vui lĂ²ng xĂ¡c nháº­n láº¡i.',
  ...
  if (allowedHostnames.size && !allowedHostnames.has(hostname)) {
    logger.warn(
      { requestId: req?.id, hostname, allowedHostnames: [...allowedHostnames] },
      'Google reCAPTCHA hostname mismatch',
    );
    throw recaptchaError(
      422,
      `TĂªn miá»n reCAPTCHA khĂ´ng khá»›p (${hostname || 'khĂ´ng xĂ¡c Ä‘á»‹nh'}). Kiá»ƒm tra RECAPTCHA_ALLOWED_HOSTNAMES trĂªn backend.`,
...
const serializeQuote = (quote) => ({
  ...
  items: (quote.items || []).map((item) => ({
    ...
    unitPrice: serializeNumber(item.unitPrice),
    quantity: item.quantity,
    lineTotal: serializeNumber(item.lineTotal),
  })),
});
...
export const quoteService = {
  async create({ items, note, intent, requestId, recaptchaToken }, req) {
    if (intent !== 'MESSENGER')
      throw ApiError.unprocessable(
        'Phiáº¿u chá»‰ Ä‘Æ°á»£c ghi nháº­n khi khĂ¡ch gá»­i qua Messenger.',
      );
    ...
    if (existing) {
      if (existing.archivedAt)
        throw ApiError.conflict(
          'Phiáº¿u nĂ y Ä‘Ă£ Ä‘Æ°á»£c lÆ°u trá»¯. Vui lĂ²ng táº¡o láº¡i tá»« giá» hĂ ng.',
        );
      const existingToken = decryptQuoteToken(existing.publicTokenCiphertext);
      if (!existingToken)
        throw ApiError.conflict('KhĂ´ng thá»ƒ khĂ´i phá»¥c link phiáº¿u Ä‘Ă£ táº¡o.');
      return quoteResponse(existing, existingToken);
    }

    const sessionHash = sessionHashForRequest(req);
    const recentCount = await prisma.interestEvent.count({
      where: {
    ...
    if (recentCount >= DAILY_QUOTE_LIMIT)
      throw new ApiError(
        429,
        'Báº¡n Ä‘Ă£ gá»­i quĂ¡ nhiá»u phiáº¿u trong hĂ´m nay. Vui lĂ²ng liĂªn há»‡ trá»±c tiáº¿p qua Messenger.',
      );

    const uniqueIds = [...new Set(items.map((item) => item.productUuid))];
    if (uniqueIds.length !== items.length)
      throw ApiError.unprocessable(
        'Má»—i sáº£n pháº©m chá»‰ Ä‘Æ°á»£c xuáº¥t hiá»‡n má»™t láº§n trong phiáº¿u',
      );
    ...
    if (products.length !== uniqueIds.length)
      throw ApiError.unprocessable('Má»™t hoáº·c nhiá»u sáº£n pháº©m khĂ´ng cĂ²n kháº£ dá»¥ng');
    ...
    const normalized = items.map((item) => {
      ...
      if (product.stock <= 0 || item.quantity > product.stock)
        throw ApiError.unprocessable(`${product.name} khĂ´ng Ä‘á»§ tá»“n kho`);
      ...
      quote = await prisma.$transaction(async (tx) => {
        ...
        const created = await tx.quote.create({
          data: {
            ...
            publicTokenCiphertext: encryptQuoteToken(publicToken),
        ...
        await tx.quoteItem.createMany({
          data: normalized.map(({ product, quantity, unitPrice, lineTotal }) => ({
            quoteId: created.id,
        ...
        await tx.interestEvent.createMany({
          data: [
            ...
            })),
            {
              ...
              quoteId: created.id,
              sessionHash,
              metadata: { itemCount: normalized.length, intent: 'MESSENGER' },
            },
          ],
        });
  ...
  async getPublic(token) {
    ...
    if (quote.expiresAt <= new Date() && quote.status !== 'EXPIRED') {
      const expired = await prisma.quote.update({
        where: { id: quote.id },
        data: { status: 'EXPIRED' },
        include: { items: true },
      });
  ...
  async messengerOpened(token, req) {
    ...
    if (current.status === 'EXPIRED' || current.expiresAt <= new Date())
      throw ApiError.unprocessable('Phiáº¿u Ä‘Ă£ háº¿t hiá»‡u lá»±c');
    const quote = await prisma.quote.update({
      where: { id: current.id },
      data: {
        status: current.status === 'CREATED' ? 'MESSENGER_OPENED' : current.status,
        messengerOpenedAt: current.messengerOpenedAt || new Date(),
      },
      include: { items: true },
    });
    await logEvent({
      eventType: 'MESSENGER_CLICKED',
      quoteId: current.id,
      sessionHash: sessionHashForRequest(req),
    });
    return serializeQuote(quote);
  },
  ...
  async track({ eventType, productUuid, metadata }, req) {
    const product = await prisma.product.findFirst({
      where: { uuid: productUuid, deletedAt: null },
      select: { id: true },
    });
    ...
    await logEvent({
      eventType,
      productId: product.id,
      sessionHash: req?.ip ? hashToken(req.ip) : null,
