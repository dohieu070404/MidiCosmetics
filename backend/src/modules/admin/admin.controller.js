...
const ok = (res, message, data = {}, meta = {}) =>
  res.success({ statusCode: HTTP_STATUS.OK, message, data, meta });
...
export const adminController = {
  ...
  permissions: asyncHandler(async (req, res) =>
    ok(res, 'Admin permissions fetched successfully', { user: req.user }),
  ),
  ...
  requestPasswordChange: asyncHandler(async (req, res) => {
    try {
      ...
      if (!emailResult?.ok) {
        throw new Error(
          'Không gửi được email xác minh đổi mật khẩu. Vui lòng kiểm tra cấu hình SMTP.',
        );
      }
      return ok(res, 'Vui lòng kiểm tra email để xác minh đổi mật khẩu.', {
        expiresAt: result.expiresAt,
      });
    } catch (error) {
      await auditService.log(req, {
  ...
  createNotificationRecipient: asyncHandler(async (req, res) => {
    const result = await adminNotificationRecipientService.createRecipient(
      req,
      req.user,
      req.validated.body,
    );
    ...
    return created(
      res,
      'Email nhận thông báo đã được thêm. Vui lòng kiểm tra email để lấy mã xác minh.',
      {
  ...
  verifyNotificationRecipient: asyncHandler(async (req, res) => {
    const recipient = await adminNotificationRecipientService.verifyRecipient(
      req.user,
      req.validated.body,
    );
    await withAudit(req, {
  ...
  toggleNotificationRecipient: asyncHandler(async (req, res) => {
    const recipient = await adminNotificationRecipientService.toggleRecipient(
      req.validated.params.uuid,
    );
    await withAudit(req, {
    ...
    return ok(
      res,
      recipient.isActive
        ? 'Email nhận thông báo đã được bật.'
        : 'Email nhận thông báo đã được tắt.',
      { recipient },
    );
  }),

  deleteNotificationRecipient: asyncHandler(async (req, res) => {
    const recipient = await adminNotificationRecipientService.deleteRecipient(
      req.validated.params.uuid,
    );
    await withAudit(req, {
  ...
  updateHomepageSection: asyncHandler(async (req, res) => {
    ...
    const section = await adminHomepageService.updateSection(
    ...
    await withAudit(req, {
      ...
      entityId: section.id,
      beforeData: before.sections.find((item) => item.id === section.id),
      afterData: section,
    });
  ...
  toggleHomepageSection: asyncHandler(async (req, res) => {
    const before = await adminHomepageService.getHomepageSettings();
    const section = await adminHomepageService.toggleSection(
      req.validated.params.sectionId,
      req.validated.body,
    );
    await withAudit(req, {
    ...
    return ok(res, section.isEnabled ? 'Section đã được bật.' : 'Section đã được tắt.', {
      section,
    });
  }),
  ...
  addHomepageFeaturedItem: asyncHandler(async (req, res) => {
    const item = await adminHomepageService.addFeaturedItem(
      req.validated.params.sectionId,
      req.validated.body,
    );
    await withAudit(req, {
      action: 'HOMEPAGE_FEATURED_ITEM_ADDED',
      entityType: req.validated.body.entityType,
      entityId: item.uuid,
      afterData: {
        uuid: item.uuid,
  ...
  removeHomepageFeaturedItem: asyncHandler(async (req, res) => {
    const item = await adminHomepageService.removeFeaturedItem(
      req.validated.params.sectionId,
      req.validated.params.itemId,
    );
    await withAudit(req, {
      action: 'HOMEPAGE_FEATURED_ITEM_REMOVED',
      entityType: 'HomepageFeaturedItem',
      entityId: item.uuid,
      afterData: { uuid: item.uuid, isFeatured: item.isFeatured },
    });
    return ok(res, 'Featured item removed successfully', { item });
  }),

  reorderHomepageFeaturedItems: asyncHandler(async (req, res) => {
    await adminHomepageService.reorderFeaturedItems(
      req.validated.params.sectionId,
      req.validated.body,
    );
    await withAudit(req, {
      action: 'HOMEPAGE_FEATURED_ITEM_REORDERED',
      entityType: 'HomepageFeaturedItem',
      entityId: req.validated.params.sectionId,
      afterData: req.validated.body.items,
    });
    return ok(res, 'Featured items reordered successfully', {});
  }),

  dashboard: asyncHandler(async (req, res) =>
    ok(res, 'Dashboard fetched successfully', await adminService.getDashboard()),
  ),
  ...
  deleteBlogCategory: asyncHandler(async (req, res) => {
    ...
    await withAudit(req, {
      ...
      entityType: 'BlogCategory',
      entityId: req.validated.params.uuid,
    });
  ...
  getBlogPost: asyncHandler(async (req, res) =>
    ok(res, 'Blog post fetched successfully', {
      post: await adminService.getBlogPost(req.validated.params.uuid),
    }),
  ),
  ...
  updateBlogPost: asyncHandler(async (req, res) => {
    const post = await adminService.updateBlogPost(
      req.user,
      req.validated.params.uuid,
      req.validated.body,
    );
  ...
  publishBlogPost: asyncHandler(async (req, res) =>
    ok(res, 'Blog post published successfully', {
      post: await adminService.setBlogPostStatus(req.user, req.validated.params.uuid, 'PUBLISHED'),
    }),
  ),
  unpublishBlogPost: asyncHandler(async (req, res) =>
    ok(res, 'Blog post unpublished successfully', {
      post: await adminService.setBlogPostStatus(req.user, req.validated.params.uuid, 'DRAFT'),
    }),
  ),
  archiveBlogPost: asyncHandler(async (req, res) =>
    ok(res, 'Blog post archived successfully', {
      post: await adminService.setBlogPostStatus(req.user, req.validated.params.uuid, 'ARCHIVED'),
    }),
  ),
  updateBlogPostFeatured: asyncHandler(async (req, res) => {
    ...
    const post = await adminService.setBlogPostFeatured(
      req.validated.params.uuid,
      req.validated.body,
    );
    await withAudit(req, {
      action: 'UPDATE_BLOG_POST_FEATURED',
      entityType: 'BlogPost',
      entityId: post.uuid,
      beforeData: beforePost,
      afterData: post,
    });
    return ok(
      res,
      post.isFeatured
        ? 'Bài viết đã được đưa lên trang chủ.'
        : 'Bài viết đã được bỏ khỏi đề xuất trang chủ.',
      { post },
    );
  }),
  ...
  listCollections: asyncHandler(async (req, res) => {
    ...
    return ok(
      res,
      'Product collections fetched successfully',
      { collections: result.items },
      result.pagination,
    );
  }),
  getCollection: asyncHandler(async (req, res) =>
    ok(res, 'Collection fetched successfully', {
      collection: await adminService.getCollection(req.validated.params.uuid),
    }),
  ),
  ...
  setCollectionProducts: asyncHandler(async (req, res) => {
    const collection = await adminService.setCollectionProducts(
      req.validated.params.uuid,
      req.validated.body.items,
    );
    await withAudit(req, {
      action: 'UPDATE_COLLECTION_PRODUCTS',
      entityType: 'ProductCollection',
      entityId: collection.uuid,
      afterData: { productUuids: collection.products.map((item) => item.product.uuid) },
    });
  ...
  downloadInventoryExport: asyncHandler(async (req, res) => {
    ...
    await withAudit(req, {
      action: 'EXPORT_PRODUCT_INVENTORY',
      entityType: 'Product',
      metadata: { filename: inventory.filename },
    });
  ...
  archiveProductsBulk: asyncHandler(async (req, res) => {
    ...
    await withAudit(req, {
      action: 'ARCHIVE_PRODUCTS_BULK',
      entityType: 'Product',
      afterData: result,
      metadata: { uuids: req.validated.body.uuids },
    });
  ...
  permanentlyDeleteProductsBulk: asyncHandler(async (req, res) => {
    ...
    await withAudit(req, {
      action: 'PERMANENT_DELETE_PRODUCTS_BULK',
      entityType: 'Product',
      afterData: result,
      metadata: { uuids: req.validated.body.uuids },
    });
  ...
  getProduct: asyncHandler(async (req, res) =>
    ok(res, 'Product fetched successfully', {
      product: await adminService.getProduct(req.validated.params.uuid),
    }),
  ),
  ...
  deleteProduct: asyncHandler(async (req, res) => {
    ...
    await withAudit(req, {
      ...
      entityType: 'Product',
      entityId: beforeProduct.uuid,
      beforeData: beforeProduct,
      afterData: { ...beforeProduct, status: 'ARCHIVED', deletedAt: new Date() },
    });
  ...
  updateProductFeatured: asyncHandler(async (req, res) => {
    const beforeProduct = await adminService.getProduct(req.validated.params.uuid);
    const product = await adminService.setProductFeatured(
      req.validated.params.uuid,
      req.validated.body,
    );
    await withAudit(req, {
      action: 'UPDATE_PRODUCT_FEATURED',
      entityType: 'Product',
      entityId: product.uuid,
      beforeData: beforeProduct,
      afterData: product,
    ...
    return ok(
      res,
      product.isFeatured
        ? 'Sản phẩm đã được đưa lên trang chủ.'
        : 'Sản phẩm đã được bỏ khỏi đề xuất trang chủ.',
      { product },
    );
  }),
  ...
  getQuote: asyncHandler(async (req, res) =>
    ok(res, 'Chi tiết báo giá đã được tải.', {
      quote: await adminOperationsService.getQuote(req.validated.params.uuid),
    }),
  ),
  updateQuoteStatus: asyncHandler(async (req, res) => {
    const quote = await adminOperationsService.updateQuoteStatus(
      req.validated.params.uuid,
      req.validated.body.status,
    );
    await withAudit(req, {
  ...
  archiveQuotes: asyncHandler(async (req, res) => {
    ...
    await withAudit(req, {
      action: 'ARCHIVE_QUOTES_BULK',
      entityType: 'Quote',
      afterData: result,
      metadata: { mode: req.validated.body.mode, uuids: req.validated.body.uuids },
    });
  ...
  interestAnalytics: asyncHandler(async (req, res) =>
    ok(
      res,
      'Dữ liệu quan tâm đã được tải.',
      await adminOperationsService.interestAnalytics(req.validated.query),
    ),
  ),
  ...
  listSettings: asyncHandler(async (req, res) =>
    ok(res, 'Settings fetched successfully', {
      settings: await adminService.listSettings(req.validated.query || {}),
    }),
  ),
